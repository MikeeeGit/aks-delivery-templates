...
The maintained platform profile installs pinned **Envoy Gateway / Gateway API**, private per-slot frontends, workload ServiceAccounts and reviewed common manifests. A separate retired NGINX compatibility profile records the original controller settings for migration. Application delivery remains Kustomize: configuration/CSI, immutable image rendering, approved apply, rollout, and selected-slot Service plus HTTPS Gateway verification. Application deployment and traffic cutover have separate approvals.
