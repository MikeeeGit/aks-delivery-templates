...
This adds Argo CD to each cluster after the existing Terraform, namespace/RBAC, and Envoy platform stages. It does not replace those stages or the direct delivery pipelines. Argo CD reconciles the same reviewed application Kustomize output. Choose one application delivery owner per slot; do not run direct apply and Argo against the same resources simultaneously.
...
The cache is limited to the named application namespace and the listed resource kinds; strict RBAC discovery and annotation-based resource tracking are enabled. Annotation tracking preserves existing slot labels. The built-in default AppProject denies every source/destination. The application delivery helper supplies an explicitly scoped project and Application. Kubernetes Roles and AppProject restrictions protect different boundaries; protect write access to both.
