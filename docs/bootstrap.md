...
For managed-Entra native authorization, follow the optional [first-time platform CI bootstrap](platform-ci-bootstrap.md) before running cluster-wide platform installation through CI. The existing operator grants the dedicated platform identity access to explicitly selected slots; this is separate from the application's namespace Role/RoleBinding.
