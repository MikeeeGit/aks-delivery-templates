...
Retain the successful receipt's source commit and digest. Render offline for one slot:
...
Local apply confirms interactively. `--yes` is an explicit automation opt-in behind external CI approval. The helper checks the namespace's default ServiceAccount, checks permissions, performs server dry-run, applies and waits ten minutes for rollout. Configured verification checks readiness and exact source revision/slot through the selected Service. With `verification.ingress`, it additionally requires current-generation Gateway/HTTPRoute conditions for all declared listeners and verifies HTTPS through that cluster's Envoy Service using actual Host/SNI and certificate trust. Redirects are rejected. Apply is not transactional: inspect partial changes after failure. It does not prune removed resources or automatically undo a failed release.

Use the selected-slot wrapper for `aks01`, `aks02` or both. Sequential mode is the default: the second slot starts only after the first completes rollout and configured verification successfully. Explicit parallel mode has independent per-slot approval/results and no cross-slot failure rollback. Verify actual load balancer IPs and follow [promotion](promotion.md) for traffic cutover. Hosted tests cannot prove live Azure/API/app behavior.
